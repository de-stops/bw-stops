...
const endpointBase = "http://www.efa-bw.de/nvbw/XSLT_COORD_REQUEST?&jsonp=&boundingBox=&boundingBoxLU={minx}%3A{miny}%3AWGS84%5BDD.DDDDD%5D&boundingBoxRL={maxx}%3A{maxy}%3AWGS84%5BDD.DDDDD%5D&coordOutputFormat=WGS84%5BGGZHTXX%5D&type_1=STOP&outputFormat=json&inclFilter=1";
...
const queryStops = function(minx, miny, maxx, maxy, callback) {
	...
	got(url).then(response => {
		...
		console.log("Got " + result.pins.length + " results from URL " + url + ".");
...
const retrieveStops = function(minx, miny, maxx, maxy) {
	...
	const callback = s => {
		...
		if (existingStop &&(
			...
			/*
			console.log("Found existing stop with id " + stopId);
			console.log("Existing stop:");
